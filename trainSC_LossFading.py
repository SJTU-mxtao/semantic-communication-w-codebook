...
def train_one_epoch(args, lambda_loss_local, H_fading_all):
    ...
    if args.trainset == 'CIFAR10' or args.trainset == 'STL10':
        for batch_idx, (input, label) in enumerate(train_loader): 
            ...
            # search for the codeword (这里search也不只是MSE，如果其中一项置零了就是MSE)
            code_assist = input.clone() 
            code_index = []  # 统计每个数据样本对应的码字W
            ...
            loss = (loss_G - lambda_loss_local * loss_P).clone()  # 做归一化防止出现负数的loss
            ...
            if epoch >= 40 and epoch % 20 == 0:
                ...
                if batch_idx % 50 == 0 and epoch % 5 == 0:
                    ...
                    for iii in range(input.size()[0]):
                        # save_image(recon_image0[iii], ('./image_raw/img%d_epoch%d_batch%d_snr%d.png' % (iii, epoch, batch_idx, snr))) 
        ...
        with torch.no_grad():
            if args.trainset == 'CIFAR10' or args.trainset == 'STL10':
                for batch_idx, (input, label) in enumerate(test_loader):
                    ...
                    # code_index_distribution = np.zeros(codebook.size()[0])  # 统计W的概率
                    code_index = []  # 统计每个数据样本对应的码字W
                    for image_ID in range(input.size()[0]):
                        code_index_local = 0
                        mse_ini = 10 ** 8
...
if __name__ == '__main__':
    ...
    if args.channel_type == 'awgn':
        ...
        model_path = "./saved_model/awgn/STL10/SC_loss_snr" + str(snr_ini) + "_psnr_C" + str(args.C) + ".model"
