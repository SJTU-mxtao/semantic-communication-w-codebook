...
class WITT_Encoder(nn.Module):
    ...
    def __init__(self, img_size, patch_size, in_chans,
        ...
        # 不用混合了，还是先求个残差，然后再对残差结果进行处理，不也是达到了一开始非线性处理的效果
        # 还有个问题，为啥不直接来论文里提，可以直接用预训练模型加载到部分层上？可能这个在工程上是可以的，但是在现实场景中没必要这样迁移的，没道理这么做

        # self.tconv1 = nn.ConvTranspose2d(6, 3, kernel_size=3, stride=1, padding=0)
        ...
        self.FCMean = torch.nn.Linear(int(256*C), int(256*C))  # 如果太大了就别用全连接了。。虽然理论上全连接确实效果好一些，改成两路3个反卷积和卷积
